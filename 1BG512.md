...
So,if we want to test the `Bezan` and see if they are admixed with `GaAdangbe `, `Sotho` & `Zulu` (you always need atleast 3 sources) then our parameters file would look as follows:
