...
Your overarching goal for these two afternoons is to gather information about your given populations so that you can answer the following questions about the Central African Rainforest Hunter-gatherers:
	...
	3	One of the RHGs populations astands out from the others. Why do think that is?
...
Rainforest Hunter-Gatheres - [Patin et al 2017](https://science.sciencemag.org/content/356/6337/543.long) 
...
```
cd /proj/g2019012/nobackup/PLINK_workshop #Uppmax project for this course
mkdir PopGen19_YOURNAME
```
...
```
/proj/g2019012/nobackup/PLINK_workshop
```
...
This will create a copy of the bim file which you will have to rename to take the place of the original file, os the move command `mv`: 
...
## OPTIONAL 1 - Data Merging and strand flipping.
...
In case you didn’t go through OPTIONAL 1, please copy these files into your folder:

```
cp ../DATA/MMerged_data.bed .
...
k2_1 <- read.table("Merged_data.2.Q.1")
k2_2 <- read.table("Merged_data.2.Q.2")
k2_3 <- read.table("Merged_data.2.Q.3")
k3_1 <- read.table("Merged_data.3.Q.1")
k3_2 <- read.table("Merged_data.3.Q.2")
k3_3 <- read.table("Merged_data.3.Q.3")
k4_1 <- read.table("Merged_data.4.Q.1")
k4_2 <- read.table("Merged_data.4.Q.2")
k4_3 <- read.table("Merged_data.4.Q.3")
k5_1 <- read.table("Merged_data.5.Q.1")
k5_2 <- read.table("Merged_data.5.Q.2")
k5_3 <- read.table("Merged_data.5.Q.3")
k6_1 <- read.table("Merged_data.6.Q.1")
k6_2 <- read.table("Merged_data.6.Q.2")
k6_3 <- read.table("Merged_data.6.Q.3")
...
This creates the pdf `Admixture_Plot1.pdf`. The bar plots have the individual K cluster asignment for the 3 iterations at K=2-6. The order of individuals is in file “Merged_data.fam”
...
We will now plot the PCs using R.
...
So,if we want to test the `Bezan` and see if they are admixed with `GaAdangbe ` & `Zulu` then our parameters file would look as follows:
