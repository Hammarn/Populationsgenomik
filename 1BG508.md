...
```
cd /proj/g2020001/nobackup #Uppmax project for this course
...
When PONG is done it will start hosting a webserver wich displays the results at port 4000 by default:  http://localhost:4000. Pong needs to be running for you to look at the results, i.e. if you close it it will not work..
...
Make sure that you connect to the same rackham (i.e 1,2,3 etc) as you got form hostename.  
In a new tab (if you didn't put PONG in the background) type:

```
firefox http://localhost:YOUR_PORT_NUMBE0R
```
...
```
This is the end of section 2. Did you figure out who the unknow populations where?!
